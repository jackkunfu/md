...
- HTTPS
  ...
  - 一个 HTTPS 请求实际上包含了两次 HTTP 传输，步骤
    ...
    - 客户端发起 https 请求，访问服务端 443 端口，给出协议版本号、支持的加密算法、随机数（Client random）等
    - 服务器确认双方使用的加密方法，服务端有密钥对，即公钥和私钥，把公钥证书以及一个服务器生成的随机数（Server random）发给客户端
    ...
    - 证书无误之后，对称加密密钥生成
      ...
      - 一种说法
        - 那么客户端会生成一个随机值，这个随机值就是用于进行对称加密的密钥
      - 另一种说法
        - 客户端请求时，生成一个随机数 Client random
        - 服务端响应确认加密算法，发送公钥的时候，生成一个随机数 Server random
        - 客户端获取公钥后，再生成一个随机数，运用非对称加密处理发送给服务端，服务端用私钥解密获取到此随机数
        - 最终客户端、服务端根据上面三个随机数生成对话密钥（session key）
  ...
  - 断开了恢复方式
    ...
    - session ID
      - 对话中断重连的时候，客户端给出这个编号，服务器有这个编号的记录，双方就可以重新使用已有的"对话密钥"，而不必重新生成
      - 缺点：
        - session ID 一般只保留在一台服务器上，多服务器处理，请求到别的服务器就会无法恢复
    - session ticket
      - 兼容性不高，目前只有 chrome firefox 支持

  - 中间人攻击

    - RSA 非对称加密流程中的
    - 中间人拦截服务端发来的公钥，把中间人的公钥发送给客户端
    - 客户端获取中间人的公钥以为是服务端公钥，利用中间人公钥加密数据发送
    - 中间人拦截到数据，用自中间人自己的私钥解密获取信息，再通过服务端的公钥加密，发送给服务端

    - 解决
      - 主要确认对面的人是你想通信的人
      - 服务端把公钥喝个人信息内容生成一个 hash，如果被中间人改动一点点，最终的 hash 都会不一致
        - 这样可以防止修改原始内容
        - 但是中间人可以把整个信息都完全替换，生成新的摘要 hash
      - 进一步解决
        - 让有公信力的认证中心（简称 CA）用它的私钥对消息摘要加密，形成签名
        - 还把原始信息和数据签名合并， 形成一个全新的东西，叫做“数字证书”
        - 客户端验证
          - 获取到证书后用同样的 hash 算法，再次生成摘要
          - 用 CA 的密钥对数字证书进行解密得到 CA 创建的摘要
          - 两个对比即可直到是否被篡改
